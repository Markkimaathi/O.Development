...
class PurchaseRequestLineMakeRFQ(models.TransientModel):
    ...
    sync_data_planned = fields.Boolean(
        ...
        help=(
            "When checked, RFQ lines on the selected RFQ are only reused "
    ...
    def check_group(self, request_lines):
        if len(list(set(request_lines.mapped("request_id.group_id")))) > 1:
            raise UserError(
                _(
                    "You cannot create a single RFQ from "
                    "purchase requests that have different procurement groups."
                )
            )
    ...
    def create_allocation(self, rfq_line, pr_line, new_qty, alloc_uom):
        vals = {
            ...
            "rfq_id": rfq_line.rfq_id.id,
        }
        return self.env["purchase.request.allocation"].create(vals)

    @api.model
    def _prepare_rfq_line(self, rfq, item):
    ...
    @api.model
    def _get_rfq_line_name(self, rfq, line):
    ...
    def _get_rfq_line_search_domain(self, rfq, item):
        vals = self._prepare_rfq_line(rfq, item)
        name = self._get_rfq_line_name(rfq, item)
        rfq_line_data = [
            ("order_id", "=", rfq.id),
            ("name", "=", name),
